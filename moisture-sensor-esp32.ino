...
// FOTA connects only to GitHub's API/web host (github.com / api.github.com,
// Sectigo -> USERTrust ECC) and its release-asset CDN (objects.
// githubusercontent.com / release-assets.githubusercontent.com, Let's
// Encrypt -> ISRG Root X1). Trusting just those two roots instead of the
// default ~200-CA bundle saves ~65KB of flash. Verified 2026-09-11 by
// validating both live chains against these exact roots with openssl.
// If GitHub changes CA/CDN provider, FOTA will fail closed (safe) until
// this is updated with the new root.
const char FOTA_ROOT_CAS[] PROGMEM = R"pem(
-----BEGIN CERTIFICATE-----
...
#define FIRMWARE_VERSION "3.0.4-b03"
...
void buildDerivedConfig() {
  ...
  snprintf(UPDATE_TOPIC,        sizeof(UPDATE_TOPIC),      "garden/%s/update",        SENSOR_ID);
}
...
// The captive-portal page is served pre-gzipped (see captive-portal.html /
// tools/gen_config_html.py) to save flash; CONFIG_HTML_STABLE_GZ and
// CONFIG_HTML_BETA_GZ are the two "channel pre-selected" variants.
...
void handleRoot() {
  ...
  if (isBetaOrDev) {
    server.send_P(200, "text/html", (PGM_P)CONFIG_HTML_BETA_GZ, CONFIG_HTML_BETA_GZ_LEN);
  } else {
    server.send_P(200, "text/html", (PGM_P)CONFIG_HTML_STABLE_GZ, CONFIG_HTML_STABLE_GZ_LEN);
  }
}
...
void checkForUpdate() {
  ...
  client.setCACert(FOTA_ROOT_CAS);
...
static void getDeviceJSON(char* buf, size_t len) {
  snprintf(buf, len,
    "\"device\":{"
      "\"identifiers\":[\"%s\"],"
      "\"name\":\"%s\","
...
void publishDiscovery() {
  char device[256];
  getDeviceJSON(device, sizeof(device));
  ...
  snprintf(topic, sizeof(topic), "%s/binary_sensor/%s_battery_low/config", HA_DISCOVERY_PREFIX, SENSOR_ID);
  publishMqttEntity(topic, payload, sizeof(payload),
    snprintf(payload, sizeof(payload),
  ...
  snprintf(topic, sizeof(topic), "%s/button/%s_reset/config", HA_DISCOVERY_PREFIX, SENSOR_ID);
  publishMqttEntity(topic, payload, sizeof(payload),
    snprintf(payload, sizeof(payload),
  ...
  snprintf(topic, sizeof(topic), "%s/button/%s_update/config", HA_DISCOVERY_PREFIX, SENSOR_ID);
  publishMqttEntity(topic, payload, sizeof(payload),
    snprintf(payload, sizeof(payload),
...
void publishConfigDiscovery() {
  ...
  snprintf(topic, sizeof(topic), "%s/switch/%s_cfg_static_ip/config", HA_DISCOVERY_PREFIX, SENSOR_ID);
  publishMqttEntity(topic, payload, sizeof(payload),
    snprintf(payload, sizeof(payload),
...
void setup() {
  ...
  char moisturePart[96] = "";
  if (!isFirstBootDelay) {
    snprintf(moisturePart, sizeof(moisturePart),
             "\"moisture\":%d,\"moisture_raw_mv\":%d,\"dry_mv\":%d,\"wet_mv\":%d,",
             moisture.percent, moisture.rawMv, DRY_MV, WET_MV);
  } else {
    logf("State     — first cold boot, moisture omitted (sleeping %d min)\n",
         cfg.firstBootDelayMin);
  }

  char batteryPart[128] = "";
  if (batV > 0) {
    snprintf(batteryPart, sizeof(batteryPart),
             "\"battery_v\":%.2f,\"battery_pct\":%d,", batV, batPct);
  } else {
    snprintf(batteryPart, sizeof(batteryPart),
             "\"battery_v\":null,\"battery_pct\":null,");
  }

  char payload[512];
  snprintf(payload, sizeof(payload),
    "{\"sensor\":\"%s\",%s%s\"battery_raw_mv\":%d,\"rssi\":%d,\"fw_version\":\"%s\",\"ts\":\"%s\"}",
